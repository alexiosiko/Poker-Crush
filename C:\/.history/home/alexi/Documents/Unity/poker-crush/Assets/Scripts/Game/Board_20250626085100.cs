...
public class Board : MonoBehaviour
{
    ...
    async void CreateBoard()
    {
		...
		Camera.main.transform.position = new Vector3((cols - 1) * xGap / 2f, (rows - 1) * yGap / 2f, -10f);
...
{
    CardData[,] plannedBoard = new CardData[cols, rows];
    List<Task> tasks = new();

    // Step 1: Build card data
    for (int x = 0; x < cols; x++)
    {
        for (int y = 0; y < rows; y++)
        {
            ...
            do
            {
                CardData newCard = GenerateRandomCardData();
                plannedBoard[x, y] = newCard;
                attempts++;
            }
            while (WouldFormCombo(plannedBoard, x, y) && attempts < 100);
        }
    }

    // Step 2: Instantiate based on planned cards
    for (int x = 0; x < cols; x++)
    {
        for (int y = 0; y < rows; y++)
        {
            Vector2 position = IndexToPos(x, y);
            GameObject prefab = GetPrefabFromData(plannedBoard[x, y]);

            GameObject card = Instantiate(prefab, new Vector2(-2, -2), Quaternion.identity, transform);
...
{
    GameObject go = GetRandomCard();
    Card c = go.GetComponent<Card>();
    return new CardData { number = c.number, suit = c.suit };
}

bool WouldFormCombo(CardData[,] board, int x, int y)
{
    CardData c = board[x, y];

    // Check 1-card pair horizontal
    if (x > 0 && c.number == board[x - 1, y].number)
        return true;

    // Check 1-card pair vertical
    if (y > 0 && c.number == board[x, y - 1].number)
        return true;

    // (Optional) Expand to cover diagonals or more complex logic

    return false;
}

GameObject GetPrefabFromData(CardData data)
{
    var match = cards.FirstOrDefault(c =>
        c.GetComponent<Card>().number == data.number &&
        c.GetComponent<Card>().suit == data.suit
    );

    if (match == null)
    {
        Debug.LogWarning($"No prefab found for number {data.number}, suit {data.suit}. Using fallback.");
