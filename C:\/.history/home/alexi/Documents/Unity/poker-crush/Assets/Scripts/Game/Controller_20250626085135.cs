...
public class Controller : MonoBehaviour
{
	...
	public static bool menuBusy = false;
	Transform selectedCard;
    void Update()
    {
		if (Input.GetMouseButtonDown(0))
			Click();
		if (!busy && !menuBusy && Input.GetMouseButtonUp(0))
			Release();
	...
	async void Release()
	{
		...
		Score.Singleton.RemoveScore(30);
		Sound.Singleton.Play(new string[] { "move1", "move2", "move3" });

		// Fix spriterendere order so it looks like its overlapping cards
		selectedCard.GetComponent<SpriteRenderer>().sortingOrder = 1;
		swapCard.GetComponent<SpriteRenderer>().sortingOrder = 1;
