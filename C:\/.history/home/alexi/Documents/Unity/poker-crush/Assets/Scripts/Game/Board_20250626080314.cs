		...
		for (int x = 0; x < cols; x++)
		{
			for (int y = 0; y < rows; y++)
			{
				...
				do
				{
					...
					card = Instantiate(GetRandomCard(), new Vector2(-2, -2), Quaternion.identity, transform);
					card.transform.position = position; // Temporarily place it for logic check

					attempts++;
					if (attempts > 10) break; // avoid infinite loop
				}
				while (WouldFormCombo(card.transform, x, y));

				card.transform.position = new Vector2(-2, -2); // Reset pos for animation
