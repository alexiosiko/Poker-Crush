...
bool WouldFormCombo(Transform card, int x, int y)
{
    foreach (var logic in Logic.logicFunctions)
    {
        HashSet<Transform> dummy = new();
        if (logic.function(card, Vector2.right, dummy)) return true;
        if (logic.function(card, Vector2.up, dummy)) return true;
    }
    return false;
}
